    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        for part_index, raw_part in enumerate(parts):
            ...
            subchunks = (
                ...
                else self._split(part, remaining_separators[1:])
            )

            for sub_index, subchunk in enumerate(subchunks):
                joiner = separator if part_index > 0 and sub_index == 0 else " "
                candidate = f"{current_chunk}{joiner}{subchunk}" if current_chunk else subchunk
